...
class LabPulseConfig(BaseModel):
    """Validated settings for generators and workers, including cross-section links.

    Stable dictionary keys identify services and readings; live values and user-edited
    Home Assistant thresholds are held elsewhere.
    """
...
class ConfigDocument:
    """One config load: source paths, expanded YAML data and validated settings.

    Generators serialize resolved_data; workers consume the typed config model.
    Source mappings locate fragment errors. Treat nested objects as read-only.
    """
...
def render_resolved_config(document: ConfigDocument) -> str:
    """Return standalone YAML from resolved_data without writing a file.

    Fragments are already expanded; model defaults and private driver bindings
    are reconstructed when a worker loads this YAML.
    """
...
def load_config(yaml_path: str | Path = DEFAULT_CONFIG_PATH, *, text: str | None = None) -> ConfigDocument:
    """Read YAML and measurement fragments into a validated ConfigDocument.

    With text, skip reading the master file; fragments still load relative to
    yaml_path. Write nothing and report input failures as source-aware ConfigError.
    """
