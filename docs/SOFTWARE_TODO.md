...
This is the current software to-do list for the active repository-root
...
### SMS bootstrap and backlog resilience

- [ ] Decouple SMS startup from hardware-service validation so a malformed
  sensor or UPS entry cannot stop the alert-delivery worker.
- [ ] Define safe behavior for YAML syntax or top-level configuration failures,
  such as retaining a last-known-good minimal SMS/MQTT runtime configuration.
- [ ] Add an explicit creation timestamp to every SMS request and define the
  maximum useful age for each alert type.
- [ ] On MQTT reconnection, group queued requests by service and measurement
  before delivery.
- [ ] Collapse queued fault/recovery pairs into one useful current-state or
  recovery summary instead of sending both historical transitions.
- [ ] Reject stale standalone requests with an auditable log and delivery result.
- [ ] Aggregate a shared configuration or system outage into one notification
  rather than one failure and recovery message per affected service.
- [ ] Test malformed hardware config while the SMS worker remains operational,
  persistent-session backlog replay, short and long outages, and many services
  failing and recovering together.

Acceptance: an unrelated hardware configuration error cannot disable SMS, and
recovery from an SMS/MQTT outage cannot produce obsolete or repetitive message
bursts.
...
- [ ] Detect loss of the SMS worker or broker from outside the LabPulse process
  stack; an in-system alert cannot report that its own delivery path is down.
...
1. Isolate SMS startup and make queued-alert recovery safe.
2. Close the real-device reconnect and restart tests.
3. Establish correct alarm behavior across Home Assistant/Pi restarts.
4. Run the first soak test and fix what it exposes.
5. Agree temporary mute semantics and complete real-Pi subscription-command acceptance.
6. Complete backup/reconstruction and external watchdog decisions.
7. Add CI, dependency/version discipline, and a release workflow.
8. Add the open-source licence, contribution, citation, and governance files.
9. Prove the sensor-extension workflow and a second installation.
10. Implement power or fridge integration only when their requirements and
